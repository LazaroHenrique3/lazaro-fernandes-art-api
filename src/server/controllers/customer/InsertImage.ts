...
export const insertImageValidation = validation(getSchema => ({
    params: getSchema<IParamProps>(yup.object().shape({
        idCustomer: yup.number().integer().required().moreThan(0)
    })),
...
export const insertImage = async (req: Request<IParamProps>, res: Response) => {
    ...
    if (!req.params.idCustomer) {
        return res.status(StatusCodes.BAD_REQUEST).json({
            errors: {
                default: 'O parâmetro "idCustomer" precisa ser informado.'
            }
        })
    }
    ...
    const result = await CustomerProvider.insertImage(req.params.idCustomer, req.file)
