...
export const getAll = async (page: number, limit: number, filter: string, id = 0,  showInative = false): Promise<ICategory[] | Error> => {

    try {
        let resultSearchFilter = await CategoryUtil.getCategoriesWithFilter(filter, page, limit, showInative)
