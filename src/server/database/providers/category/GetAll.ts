...
//Funções auxiliares
import { getCategoryById, getCategoriesWithFilter } from './util'
...
export const getAll = async (page: number, limit: number, filter: string, id = 0): Promise<ICategory[] | Error> => {
    ...
    try {
        let resultSearchFilter = await getCategoriesWithFilter(filter, page, limit)
        ...
        if (id > 0 && resultSearchFilter.every(item => item.id !== id)) {
            const resultById = await getCategoryById(id)
