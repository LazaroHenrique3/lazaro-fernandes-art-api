...
export const getAll = async (page: number, limit: number, filter: string, status: string, id = 0, showInative = false): Promise<IDimension[] | Error> => {

    try {
        let resultSearchFilter = await DimensionUtil.getDimensionsWithFilter(filter, page, limit, status, showInative)
