...
//Funções auxiliares
import { getDimensionById, getDimensionsWithFilter } from './util'
...
export const getAll = async (page: number, limit: number, filter: string, id = 0): Promise<IDimension[] | Error> => {
    ...
    try {
        let resultSearchFilter = await getDimensionsWithFilter(filter, page, limit)
        ...
        if (id > 0 && resultSearchFilter.every(item => item.id !== id)) {
            const resultById = await getDimensionById(id)
