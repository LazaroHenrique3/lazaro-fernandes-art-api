...
export const getAll = async (page: number, limit: number, filter: string, category: string, technique: string, order: string, type: string, status: string, id = 0): Promise<IProduct[] | Error> => {

    try {
        let resultSearchFilter = await ProductUtil.getProductsWithFilter(filter, category, technique, order, type, status, page, limit)
