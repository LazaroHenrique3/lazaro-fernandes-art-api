...
export const getAll = async (page: number, limit: number, filter: string, category: string, technique: string, id = 0): Promise<IProduct[] | Error> => {

    try {
        let resultSearchFilter = await ProductUtil.getProductsWithFilter(filter, category, technique, page, limit)
