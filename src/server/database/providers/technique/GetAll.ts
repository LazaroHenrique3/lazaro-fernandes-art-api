...
export const getAll = async (page: number, limit: number, filter: string, id = 0, showInative = false): Promise<ITechnique[] | Error> => {

    try {
        let resultSearchFilter = await TechniqueUtil.getTechniquesWithFilter(filter, page, limit, showInative)
